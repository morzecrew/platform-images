...
does not flatten honestly, so there is no `CH_CONF__` channel over it. But
*settings* — the profile-level knobs like `max_execution_time`,
`max_bytes_before_external_group_by` — are genuinely flat key-value, so
`CH_SETTING__<name>` mapping into the default profile is coherent and is the
right shape for RFC 0001's passthrough channel. One image, two config surfaces,
and only one of them gets a passthrough. The README must be explicit about which,
or the absence of `CH_CONF__` reads as an oversight.
...
| 7 | `ASSUMED` | `CH_SETTING__<name>` passthrough over profile settings, and no passthrough over server config (§5.4). Depart if profile settings turn out not to be as flat as they look. |
