...
| [0004](0004-postgres-extensions-as-a-build-argument.md) | Postgres extensions as a build argument | 🚧 In progress | Adding a Postgres extension currently means a second image directory; a `PG_EXTENSIONS` build arg and manifest that also generates the preload line, so combinations become bake targets. |
