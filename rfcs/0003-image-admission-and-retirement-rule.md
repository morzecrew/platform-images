...
- **Status:** ✅ Complete — the rule shipped to
  ...
  **The first review fires January 2027 and has never run**; the workflow's
  scheduled path is the one thing here that cannot be exercised before then.
  GitHub's documentation states that **in a public repository, scheduled
  workflows are automatically disabled when no repository activity has occurred
  in 60 days** — which an annual cron is uniquely exposed to. The mitigation is
  manual and belongs in whoever's head runs this repo: `gh workflow enable
  "Annual Image Review"`, then trigger it with `workflow_dispatch` rather than
  waiting another year (EXECUTION-LOG D-055, R-36).
...
| 6 | ~~`OPEN`~~ **Locked 2026-08-12** | **January, triggered by a scheduled workflow that opens the review issue** with both questions and the current image list pre-filled. The month matters less than the mechanism: `cleanup-images.yaml` already proves cron works in this repo, and a self-opening issue is the difference between a review that happens and a calendar promise that does not. Owner is whoever the issue is assigned to at open time. |
